		...
		<div class="post-thumbnail-container"  <?php cryout_schema_microdata( 'image' ); ?>>
			...
			<a class="responsive-featured-image" href="<?php echo esc_url( get_permalink( $post->ID ) ) ?>" title="<?php echo esc_attr( get_post_field( 'post_title', $post->ID ) ) ?>">
				<picture>
					...
					<?php if ( cryout_on_landingpage() ) { ?><source media="" sizes="<?php echo cryout_gen_featured_sizes( $featured_width, $options['fluida_magazinelayout'], $options['fluida_landingpage'] ) ?>" srcset="<?php echo cryout_get_picture_src( $fimage_id, 'fluida-featured-lp' ); ?> <?php printf( '%sw', $featured_width ) ?>">
					<?php } ?>
